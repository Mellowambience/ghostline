"""ghostdns config loader.

Loads configuration from ``~/.ghostline/ghostdns/config.yaml``,
...
DEFAULT_CONFIG_PATH = Path.home() / ".ghostline" / "ghostdns" / "config.yaml"
...
def load_config(path: Path = DEFAULT_CONFIG_PATH) -> dict:
    """Load and return the ghostdns config."""
    if not path.exists():
        raise FileNotFoundError(
            f"Config not found at {path}.\n"
            "Copy ghostdns/config.example.yaml to get started."
        )

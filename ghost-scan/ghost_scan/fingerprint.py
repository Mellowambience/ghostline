"""Service fingerprinting for ghost_scan.

Maps well-known TCP port numbers to service names and optionally grabs
service banners from open ports.

The PORT_SERVICES table below is derived from the Nmap service database and
covers all 8,000+ registered TCP services so classification is accurate.
classify_port() returns "unknown" for any port not present.
"""

# port -> service name (from nmap-services, TCP)
PORT_SERVICES = {
    1: 'tcpmux',
    2: 'compressnet',
    3: 'compressnet',
...
def classify_port(port: int) -> str:
    ...
    Returns "unknown" if the port is not in the table.
    """
    return PORT_SERVICES.get(int(port), "unknown")


def grab_banner(host: str, port: int, timeout: float = 0.5) -> str:
    """Attempt to grab a service banner from an open (host, port).

    Connects, reads a few bytes, decodes as UTF-8 (falling back to latin-1),
    and truncates to 200 characters (plus an ellipsis if longer).

    Returns an empty string if no banner is received or an error occurs.
    """
    import socket
